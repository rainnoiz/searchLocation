...
(function ($) {
    ...
    var _methods = {
        init: function (options) {
            ...
            if (target.length > 0) {
                ...
                tbHead.className = 'appendGrid-ui-widget-header';
                tbBody = document.createElement('tbody');
                tbBody.className = 'appendGrid-ui-widget-content';
                tbFoot = document.createElement('tfoot');
                tbFoot.className = 'appendGrid-ui-widget-footer';
                // Remove existing content and append new thead and tbody
                $(tbWhole).empty().addClass('appendGrid').addClass('appendGrid-ui-widget').append(tbHead, tbBody, tbFoot);
                // Handle header row
                ...
                if (!settings.hideRowNumColumn) {
                    ...
                    tbCell.className = 'appendGrid-ui-widget-header';
    ...
    function insertRow(tbWhole, numOfRow, rowIndex, callerUniqueIndex) {
        ...
        for (var z = 0; z < numOfRow; z++) {
            ...
            if (!settings.hideRowNumColumn) {
                tbCell = tbRow.insertCell(-1);
                $(tbCell).addClass('appendGrid-ui-widget-content first').text(settings._rowOrder.length);
            }
            ...
            for (var y = 0; y < settings.columns.length; y++) {
                ...
                // Insert cell
                tbCell = tbRow.insertCell(-1);
                tbCell.className = 'appendGrid-ui-widget-content';
